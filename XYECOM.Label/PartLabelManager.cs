    ...
    public class PartLabelManager : LabelManger
    {
        ...
        private string Analysis(Model.PartLabelInfo labelInfo)
        {
            ...
            foreach (Match m in linkRegx.Matches(baseHTML))
            {
                ...
                {
                    Model.UserNewsInfo newsInfo = new Business.UserNews().GetItem((int)infoId);
                    ...
                    {
                        long userId = newsInfo.UserId;

                        Model.GeneralUserInfo userinfo = Business.CheckUser.GetUserInfo(userId);

                        if (Business.UserGradePopedom.IsShop(userinfo.gradeid))
                        {
                            string tempStr = userinfo.shopindex.Replace("index." + pageSuffix, "");
                            if (!tempStr.EndsWith("/")) tempStr = tempStr + "/";

                            url = string.Format(newsUrl, tempStr, infoId);
                        }
                        else
                        {
                ...
                else if (moduleName.Equals("user"))
                {
