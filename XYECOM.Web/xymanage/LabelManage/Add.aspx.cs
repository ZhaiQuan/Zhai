        ...
        if (lid > 0 && (el = le.GetItem(lid)) != null)
        {
            ...
            switch (el.LabelRange)
            {
                ...
                case XYECOM.Model.LabelRange.UserGropu:
                    ...
                    XYECOM.Business.UserGrade userGradeBll = new XYECOM.Business.UserGrade();
                    List<XYECOM.Model.UserGradeInfo> gradeList = userGradeBll.GetItems(el.GroupIdOrUserId);
                    string gradeNames = string.Empty;
                    foreach (XYECOM.Model.UserGradeInfo info in gradeList)
                    {
                        gradeNames += string.Format("{0},", info.GradeName);
                    }
                    this.spanGroupNames.InnerHtml = XYECOM.Core.Utils.RemoveEndComma(gradeNames);
