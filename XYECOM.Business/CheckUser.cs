...
namespace XYECOM.Business
{
    ...
    public class CheckUser
    {
                    ...
                    if ("" != strUserName)
                    {
                        ...
                        if (null != _userInfo)
                        {
                            ...
                            if (null != _info)
                            {
                                ...
                                XYECOM.Core.Utils.WriteCookie("UserLevel", _userInfo.GradeInfo.GradeName.ToString(), cookieDomain);
                                XYECOM.Core.Utils.WriteCookie("UserLevelImg", _userInfo.GradeInfo.SmallIconName, cookieDomain);

                                string strUserType = "user";
                                if (_userInfo.Type == false) strUserType = "person";
        ...
        public static XYECOM.Model.GeneralUserInfo GetUserInfo(XYECOM.Model.UserRegInfo userRegInfo)
        {
            ...
            _templateuserinfo.usertype = userRegInfo.Type;
            ...
            if (userRegInfo.Type)
            {
                ...
                if (_UserInfo != null)
                {
                    ...
                    _templateuserinfo.isshop = userRegInfo.GradePopedomInfo.UGP_IsShop;
                    _templateuserinfo.isbindingtopdomain = userRegInfo.GradePopedomInfo.IsBindingTopdomain;
                    _templateuserinfo.issubdomain = userRegInfo.GradePopedomInfo.IsSubdomain;
                    ...
                    _templateuserinfo.iscompanyprotype = userRegInfo.GradePopedomInfo.IsCompanyProType;
                    ...
                    _templateuserinfo.gradename = _UserInfo.RegInfo.GradeInfo.GradeName;
                ...
                if (userGreadInfo != null)
                {
                    if (userGreadInfo.SmallIconName != "")
                        _templateuserinfo.gradeimgurl = webInfo.WebDomain+"icon/" + userGreadInfo.SmallIconName;

                    if (userGreadInfo.BigIconName != "")
                    {
                        _templateuserinfo.gradebigimgurl= webInfo.WebDomain+"icon/" +userGreadInfo.BigIconName;
                    }
                }
            ...
            {
                if (userRegInfo.IsHasImage)
                    _templateuserinfo.imgurl = Business.Attachment.GetInfoDefaultImgHref(XYECOM.Model.AttachmentItem.Individual, _templateuserinfo.userid);
                else
                    _templateuserinfo.imgurl = SYS_NOIMAGE_PATH;

                Model.IndividualInfo _IndividualInfo = new Business.Individual().GetItem(userRegInfo.UserId);

                _templateuserinfo.idcode = _IndividualInfo.UI_Code;

                _templateuserinfo.address = _IndividualInfo.UI_Address;

                _templateuserinfo.areaid = _IndividualInfo.AreaID;

                if (_IndividualInfo.AreaID > 0)
                {
                    Model.AreaInfo areainfo = new Business.Area().GetItem(_IndividualInfo.AreaID);
                    if (null != areainfo)
                        _templateuserinfo.areaname = areainfo.FullNameAll;
                }

                _templateuserinfo.telephone = _IndividualInfo.Telephone;

                _templateuserinfo.zipcode = _IndividualInfo.UI_Postcode;

                _templateuserinfo.mobile = _IndividualInfo.UI_Mobil;

                _templateuserinfo.realname = _IndividualInfo.UI_Name;

                _templateuserinfo.sexbool = _IndividualInfo.UI_Sex;

                _templateuserinfo.sex = "先生";

                if (!_IndividualInfo.UI_Sex) _templateuserinfo.sex = "女士";
            }
