...
namespace XYECOM.Business
{
    ...
    public class UserReg
    {
        ...
        public Model.ResisterUserReturnValue Register(Model.UserRegInfo userRegInfo, Model.UserInfo userInfo, Model.IndividualInfo individualInfo, string sourcePassword, out long userId)
        {
            userId = 0;

            if (!XYECOM.Configuration.WebInfo.Instance.IsRegister) return XYECOM.Model.ResisterUserReturnValue.ForbidRegister;

            if (null == userRegInfo) return XYECOM.Model.ResisterUserReturnValue.Failed;

            Model.UserType userType = XYECOM.Model.UserType.Enterprise;
            if (!userRegInfo.Type) userType = XYECOM.Model.UserType.Individual;

            if (userType == XYECOM.Model.UserType.Enterprise && null == userInfo) return XYECOM.Model.ResisterUserReturnValue.Failed;

            if (userType == XYECOM.Model.UserType.Individual && null == individualInfo) return XYECOM.Model.ResisterUserReturnValue.Failed;
            ...
            if (IsExistTheUserName(userName)) return XYECOM.Model.ResisterUserReturnValue.UserNameExists;

            if (IsExistTheEmail(email)) return XYECOM.Model.ResisterUserReturnValue.EmailExists;
            ...
            #region 用户注册信息
            userRegInfo.GradeId = Convert.ToInt32(webInfo.UserDefaultLevel);
            userId = 0;            
            ...
            #region 写Cookie
            if (_result <= 0) return XYECOM.Model.ResisterUserReturnValue.Failed;
            ...
            if (userRegInfo.Type)
            {
                XYECOM.Core.Utils.WriteCookie("UserType", "User", cookieDomain);
                XYECOM.Model.UserGradeInfo userGreadInfo = new XYECOM.Business.UserGrade().GetItem((short)userRegInfo.GradeId);
                if (null != userGreadInfo)
                {
                    XYECOM.Core.Utils.WriteCookie("UserLevel", userGreadInfo.GradeId.ToString(), 1, cookieDomain);
                    XYECOM.Core.Utils.WriteCookie("UserLevelImg", userGreadInfo.SmallIconName, 1, cookieDomain);
                }
            }
            else
            {
                XYECOM.Core.Utils.WriteCookie("UserType", "Person", cookieDomain);
            }
            ...
            //注册用户信息增加本站给用户的金额
            if (webInfo.UserDefaultWebMoney > 0)
            {
                new XYECOM.Business.UserFictitiouCount().UpdateUserFictitiouCount(userId, webInfo.UserDefaultWebMoney);
            }

            //增加用户资料完善度
            if (webInfo.RegisterPercent > 0)
            {
                new XYECOM.Business.UserReg().EditConsumate(userId, Convert.ToInt32(webInfo.RegisterPercent));
            }

            #region 创建用户等级关系
            ////创建用户等级关系
            XYECOM.Model.UserGradeRelationInfo ugrInfo = new XYECOM.Model.UserGradeRelationInfo();
            ugrInfo.UGR_ID = 1;
            ugrInfo.U_ID = userId;
            ugrInfo.U_BeginTime = Convert.ToDateTime(DateTime.Now.ToShortDateString());
            ugrInfo.U_EndTime = "";
            ugrInfo._U_Money = 0;
            new XYECOM.Business.UserGradeRelation().Insert(ugrInfo);
            #endregion
            ...
            #region 添加企业用户信息
            if (userType == XYECOM.Model.UserType.Enterprise)
            {
            ...
            #region 添加个人信息
            if (userType == XYECOM.Model.UserType.Individual && individualInfo != null)
            {
                individualInfo.U_ID = userId;
                new XYECOM.Business.Individual().Insert(individualInfo);
            }
            #endregion

            #region 给用户发送用户邮箱验证(企业发送，个人用户不用发送)
            bool sendMailFlag = true;
            if (webInfo.IsRegisterEmail && userRegInfo.Type)
            {
                if (email != "")
                {
            ...
            out long userId)
        {
            ...
            if (userType == XYECOM.Model.UserType.Enterprise)
                userRegInfo.Type = true;
            else
                userRegInfo.Type = false;

            #endregion

            XYECOM.Model.UserInfo userInfo = null;
            XYECOM.Model.IndividualInfo individualInfo = null;
            ...
            {
                individualInfo = new XYECOM.Model.IndividualInfo();
                individualInfo.U_ID = userId;
                individualInfo.UI_Address = "";
                individualInfo.UI_Name = linkman;
                individualInfo.UI_Sex = true;
                individualInfo.UI_Code = "";
                individualInfo.AreaID = 0;
                individualInfo.U_Email = userRegInfo.Email;
                individualInfo.UI_Flag = 1;
                individualInfo.Telephone = telephone;
                individualInfo.UI_Postcode = "";
                individualInfo.UI_Mobil = mobile;
            }
            #endregion

            return Register(userRegInfo, userInfo, individualInfo, password, out userId);
        }
        ...
        public Model.UserRegInfo Login(string userName, string password, bool isSaveInfo)
        {
            ...
            XYECOM.Model.UserRegInfo tempuserInfo = this.GetItem(userName);
            if (tempuserInfo == null) return null;

            if (tempuserInfo.Password.Trim().Length.Equals(16))
            {
                userInfo = this.GetItem(userName, XYECOM.Core.SecurityUtil.MD5(password, "16"));
            }
            else
            {
            ...
            //XYECOM.Model.UserRegInfo userInfo = this.GetItem(userName, XYECOM.Core.SecurityUtil.MD5(password, XYECOM.Configuration.Security.Instance.Md5value));

            if (userInfo == null) return null;

            this.pwd = password;
            this.ip = XYECOM.Core.XYRequest.GetIP();
            ...
            string strUserType = "person";
            if (userInfo.Type)
            {
                strUserType = "user";

                XYECOM.Model.UserGradeInfo userGreadInfo = new XYECOM.Business.UserGrade().GetItem((short)userInfo.GradeId);
                if (null != userGreadInfo)
                {
                    if (isSaveInfo)
                    {
                        XYECOM.Core.Utils.WriteCookie("UserLevel", userGreadInfo.GradeName.ToString(), 1, cookieDomain);
                        XYECOM.Core.Utils.WriteCookie("UserLevelImg", userGreadInfo.SmallIconName, 1, cookieDomain);
                    }
                    else
                    {
                        XYECOM.Core.Utils.WriteCookie("UserLevel", userGreadInfo.GradeName.ToString(), cookieDomain);
                        XYECOM.Core.Utils.WriteCookie("UserLevelImg", userGreadInfo.SmallIconName, cookieDomain);
                    }
                }
            }
            ...
            //AfterLogin(password, userInfo);

            System.Threading.Thread t1 = new System.Threading.Thread(new System.Threading.ParameterizedThreadStart(AfterLogin));
            t1.Start(userInfo);

            System.Threading.Thread t2 = new System.Threading.Thread(new System.Threading.ParameterizedThreadStart(RefreshOrder));
            t2.Start(userInfo);

            System.Threading.Thread t3 = new System.Threading.Thread(new System.Threading.ParameterizedThreadStart(BbsLogin));
            t3.Start(userInfo);
        ...
        private void RefreshOrder(object uinfo)
        {
            Model.UserRegInfo userInfo = null;
            ...
            //刷新用户交易信息
            //XYECOM.Business.OrderForm orderFormBLL = new XYECOM.Business.OrderForm();
            //orderFormBLL.Refresh(userInfo.UserId);
        }

        /// <summary>
        /// bbs 整合
        /// </summary>
        private void BbsLogin(object uinfo)
        {
            Model.UserRegInfo userInfo = null;
            ...
            if (userInfo == null || string.IsNullOrEmpty(this.pwd))
            {
                return;
            }
